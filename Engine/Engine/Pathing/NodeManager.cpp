...
EditorNode * NodeManager::getNextNode() {
	...
	nodes.push_back(ret);
	auto temp = UnSelectedColor;
	ret->rednerable = debugShapes->addUnitSphere(glm::mat4(),temp);
...
void NodeManager::selectNode(EditorNode * toSelect) {
	...
	auto temp = UnSelectedColor;
	setAllNodeColors(temp);
	currentSelectedNode = toSelect;
	currentSelectedNode->rednerable->overrideColor = SelectedColor;
	temp = ConnectedNodeColor;
	currentSelectedNode->activateConnections(temp);
}
...
void NodeManager::activateAllConnections() {
	for (uint i = 0; i < nodes.size(); i++)
	{
		auto temp = ConnectedNodeColor;
		nodes[i]->activateConnections(temp);
	}
}
...
void NodeManager::addConnection(EditorNode * fromNode, EditorNode * toNode) {
	...
	toAdd->renderable = debugShapes->addUnitVector(fromNode->pos,vectorPointer,color,1);
